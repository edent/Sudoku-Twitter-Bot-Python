...
def display(puzzle):
    ...
    for row in puzzle:
        ...
        col_counter = 0
        for n in row:
            if (col_counter%3 == 0 and col_counter > 0):
                output += u"｜"
            if (col_counter%9 == 0):
                output += u"\n"
                col_counter = 0
            col_counter += 1
            # Unicode MATHS!!!  Add 65,248 to 0-9 to transform them into the fullwidth forms.
            output += unichr(ord(str(n))+65248)
        if (row_counter == 3 or row_counter == 6):
            output += u"\n－－－－－－－－－－－"

    # 0 are blanks, so replace them with a fullwidth underscore
    return output.replace(u"０",u"＿")
...
access_token        = 'cccccccccccccccccc-ddddddddddddddddddddddddddddddd'
...
api.update_status(status=display(puzzle))
